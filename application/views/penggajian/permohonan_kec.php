                    ...
                    <table class="table table-bordered" id="example2">
                        ...
                        <tbody>
                            ...
                            foreach ($kec as $k) {
                                ...
                                //print_r($data);
                                $data = $this->permohonan_model->getPembayaranSum($k->DesaId, $bulan->BulanId, $tahun);
                                $sum_perangkat = $this->permohonan_model->perangkatSum($k->DesaId, $bulan->BulanId, $tahun);
                                $desa = $this->permohonan_model->getDesaSum($k->DesaId, $bulan->BulanId, $tahun);
                                $n4 = $data->GajiBulanan;
                            ...
                            ?>
                                ...
                                <tr>
                                    <td style="text-align: center;">
                                        <!-- <?= $data->PembayaranId; ?>
                            -->
                                        <?= approveStatusKec($data->StatusProgress, $k->DesaId, $tahun, $bulan->BulanId); ?>


                                    </td>
                                    <td style="">
                                        <a class="btn btn-info btn-rounded btn-fw detail" href="#" data-id="<?= $k->DesaId . sprintf("%02d", $bulan->BulanId) . $tahun ?>">
                                            <?= remove_word($k->DesaNama); ?>
                                        </a>
...
<script type="text/javascript">
    function approve(kec, desa, tahun, bulan) {
        function n(n) {
            return n > 9 ? "" + n : "0" + n;
        }
        var id = n(kec) + '.' + desa + '.';
    ...
    function cancel(kec, desa, tahun, bulan) {
        function n(n) {
            return n > 9 ? "" + n : "0" + n;
        }
        var id = n(kec) + '.' + desa + '.';
