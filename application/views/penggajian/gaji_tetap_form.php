    ...
    <div class="col-md-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <form class="forms-sample" action="<?=base_url("penggajian/gaji_tetap_do/$sub")?>" method="post">
            <?php
                //$CI =& get_instance();
                //$id = "aadd";
               // echo $CI->CekNik($id) ;
            ?>
                <div class="form-group row">
                    <label class="col-sm-3 col-form-label">Perangkat Desa</label>
                    <div class="col-sm-6">
                        <select name="Nik" class="form-control select2" id="Nik">

                            <option>Pilih Perangkat</option>
                            <?php foreach($perangkat as $p){
                                if($sub=="update"){
                            ?>
                            <option value="<?= $p->SiltapId?>" <?php if($p->SiltapId==$id){ echo "selected";}?>>
                                <?= $p->Nama. " - " . $p->Nik; ?>
                                </option>   
                            <?php 
                                }else{
                ...
                <div class="form-group row">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <span class="input-group-text" style="color:black;">Rp</span>
                        </div>
                        <input id="uang_sejumlah" name="GajiTetap" type="text" placeholder="Gaji Tetap" class="form-control" value="<?=$user['GajiTetap']?>"/>
                         </div>
                    </div>
                </div>
            ...
            <label class="col-sm-3 col-form-label"></label>
                <div class="col-sm-6">
                    <input type="hidden" name="SiltapId" value="<?=$id?>"/>
...
<script type="text/javascript">
        ...
        $('form').ajaxForm({
            ...
            success: function(data){
                ...
                if(data.success) {
                    location.href = '<?= base_url('penggajian/gaji_tetap')?>';
                } else{
                    $.gritter.add({
                        title: 'Error!',
                        text: data.msg,
...
$(document).ready(function(){
    $( '#uang_sejumlah' ).mask('000.000.000.000', {reverse: true});
